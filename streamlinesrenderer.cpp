...
// setter vom Mapper
void StreamLinesRenderer::setMapper(StreamLinesMapper *mapper)
{
...
void StreamLinesRenderer::drawStreamLines(QMatrix4x4 mvpMatrix, int time)
{
    int linesSize = initStreamLines(time);
...
int StreamLinesRenderer::initStreamLines(int time)
{
    std::cout << "hContourRengerer init lines \n";

    QVector<QVector3D> seedPoints;
    for(float shift=1.f; shift>=0.1f; shift*=0.8){
        seedPoints.append(QVector3D(0.7f, 0.f, 1.1-shift));
    }

    lines.clear();
    lines = streamLinesMapper->computeStreamlines(seedPoints, time);
...
//        lines.pop_front();
    ...
    if (vertexArrayObject.isCreated())
    {
        ...
        shaderProgram.setAttributeBuffer("streamlineVertexPosition", GL_FLOAT, 0, 3, 3*sizeof(float));
        ...
        //
        shaderProgram.enableAttributeArray("streamlineVertexPosition");
        vertexBuffer.release();
    }
